...
- **Setup A** (pullback) and **Setup B** (breakout retest) with per-setup configuration
...
- **Per-setup cutoff** with armed-state disarm (prevents stale entries after cutoff)
...
- **Multi-instrument chart switching** — group-selector pills let you switch between instruments (NQ, ES, GC, CL, etc.); chart loads ~200 historical bars via REST then receives live candle updates via SignalR; proper cleanup of price lines, auto-scale reset, and async race guards on instrument switch
...
- **Session badge** — dashboard shows the currently active session (Asia/London/NY) or Idle status in real-time
